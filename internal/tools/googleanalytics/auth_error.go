...
// classifyCredentialError marks only genuine credential rejections (a real
// 401, or an explicit UNAUTHENTICATED / authError body) so the engine does not
// invalidate a credential that is merely missing a scope (403), hitting a
// disabled API, or rate-limited.
