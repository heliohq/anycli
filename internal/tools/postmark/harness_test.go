...
const testToken = "server-tok-123"

// capturedRequest records what the fake Postmark server saw.
...
func newServer(t *testing.T, status int, response string, got *capturedRequest) *httptest.Server {
	...
	return httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		*got = capturedRequest{
			...
			ServerToken: r.Header.Get("X-Postmark-Server-Token"),
			Body:        body,
		}
...
func runResult(t *testing.T, srv *httptest.Server, args ...string) (execution.Result, string, string) {
	...
	result, err := svc.Execute(context.Background(), args, map[string]string{EnvServerToken: testToken})
	if err != nil {
