...
// capturedRequest records what the fake Wise server saw.
...
// run executes one wise invocation against srv with a fixed token and returns
...
func runResult(t *testing.T, srv *httptest.Server, args ...string) (execution.Result, string, string) {
	...
	result, err := svc.Execute(context.Background(), args, map[string]string{EnvAPIToken: "tok-123"})
	if err != nil {
