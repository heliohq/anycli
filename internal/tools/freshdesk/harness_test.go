...
// wantAuth is the Basic header the service must send for testAPIKey.
func wantAuth() string {
	return "Basic " + base64.StdEncoding.EncodeToString([]byte(testAPIKey+":"+dummyPassword))
}

// capturedRequest records what the fake Freshdesk server saw.
...
// with the given status/response and optional headers for every path.
func newServer(t *testing.T, status int, response string, got *capturedRequest, headers ...[2]string) *httptest.Server {
	t.Helper()
