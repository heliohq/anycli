...
// capturedRequest records what the fake SerpApi server saw.
...
// run executes one serpapi invocation against srv with the test API key set.
...
func runResult(t *testing.T, srv *httptest.Server, args ...string) (execution.Result, string, string) {
	t.Helper()
	return runWithEnv(t, srv, map[string]string{EnvAPIKey: "key-123"}, args...)
}

// runWithEnv executes one serpapi invocation with an explicit env map.
