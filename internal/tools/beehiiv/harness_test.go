...
// capturedRequest records what the fake beehiiv server saw.
...
func runResult(t *testing.T, srv *httptest.Server, args ...string) (execution.Result, string, string) {
	...
	result, err := svc.Execute(context.Background(), args, map[string]string{EnvAPIKey: "tok-123"})
	if err != nil {
...
// runNoToken executes with no credential in the environment.
