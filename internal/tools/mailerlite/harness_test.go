...
// capturedRequest records what the fake MailerLite server saw.
...
// run executes one mailerlite subcommand against the fake server with a fixed
// token in env, returning the exit code and captured stdout/stderr.
...
func runResult(t *testing.T, srv *httptest.Server, args ...string) (execution.Result, string, string) {
	...
	svc := &Service{BaseURL: srv.URL + "/api", HC: srv.Client(), Out: &out, Err: &errBuf}
	result, err := svc.Execute(context.Background(), args, map[string]string{EnvAPIToken: "ml-tok-123"})
	if err != nil {
