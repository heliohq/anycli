...
// capturedRequest records one request the fake Lemlist server received.
...
// newMux is a multi-route fake Lemlist server: it answers each request from
// routes keyed by "METHOD /path" and records every request into reqs. An
// unmatched route returns 404 so tests notice wrong paths.
