...
// child environment. The argv contains only this variable-name literal — the
// DSN value itself never appears on the command line.
...
func (s *Service) runMongosh(ctx context.Context, script, dsn string, timeout time.Duration, inv *invocation) error {
	...
	if runErr != nil {
		return classifyStderr(runErr, stderrOut)
	}
	if exitCode != 0 {
		return classifyStderr(fmt.Errorf("mongosh exited with code %d", exitCode), stderrOut)
	}
	return nil
}
...
// execMongosh is the production Runner: it resolves the pinned mongosh
// (lazy-installing from the official source on first use) and runs it with
// stdin closed (no TTY).
func execMongosh(ctx context.Context, args []string, env []string) (int, []byte, []byte, error) {
	...
	if err != nil {
		return 1, nil, nil, fmt.Errorf("resolve mongosh: %w", err)
	}
	...
	if errors.As(runErr, &exitErr) {
		// Non-zero exit is reported through exitCode + stderr, not as an
		// exec-level error.
		runErr = nil
	}
...
// classifyStderr wraps explicit provider credential rejections with
// execution.RejectCredential based on mongosh stderr text. Permission errors
// ("not authorized" / Unauthorized) and ordinary failures pass through
// unchanged — same semantics as the retired driver implementation, now judged
// from the shell's error text instead of server error codes.
func classifyStderr(err error, stderr []byte) error {
	if isAuthStderr(string(stderr)) {
		return execution.RejectCredential(err)
	}
	return err
}

func isAuthStderr(stderr string) bool {
