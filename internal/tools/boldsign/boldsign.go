// Package boldsign is the built-in BoldSign service: a non-interactive cobra
// tree over the BoldSign v1 REST surface (https://api.boldsign.com). Auth is
// "Authorization: Bearer <access_token>" (OAuth 2.0). BoldSign errors are
// non-2xx with a JSON body carrying message/error detail; a 401 rejects the
// credential. JSON-returning endpoints emit the provider JSON on stdout
// (passthrough + newline); 204 endpoints (remind, revoke) emit a small receipt;
// binary endpoints (download, audit-log) write bytes to --out and emit a
// receipt.
package boldsign
...
// (definitions/tools/boldsign.json). It is an OAuth 2.0 access token.
const EnvAccessToken = "BOLDSIGN_ACCESS_TOKEN"

// Service implements the built-in BoldSign tool. It satisfies tools.Service by
// duck typing (this package never imports the registry — no import cycle).
type Service struct {
	// BaseURL overrides the BoldSign API base; empty = DefaultBaseURL. Tests
...
// Execute runs one boldsign subcommand with the resolved credentials in env.
// Success is exit 0; usage/param errors (illegal flag combos, bad enums,
// invalid inputs, missing required flags, unknown subcommands) are exit 2;
// runtime/API errors (BoldSign non-2xx, transport failure) are exit 1. Errors
...
func (s *Service) Execute(ctx context.Context, args []string, env map[string]string) (execution.Result, error) {
	...
	if token == "" {
		...
		s.renderError(hasJSONArg(args), &usageError{msg: "BOLDSIGN_ACCESS_TOKEN is not set"})
