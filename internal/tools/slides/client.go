...
// rate-limit / server failure. Length bounds the retry count.
var retryBackoffs = []time.Duration{200 * time.Millisecond, 800 * time.Millisecond}

// call performs one Slides API request with Bearer auth. Non-2xx surfaces the
...
// 429/5xx status. Non-GET requests are never auto-retried: a batchUpdate may
...
func (s *Service) call(ctx context.Context, token, method, path string, query url.Values, payload any) ([]byte, error) {
	...
	if payload != nil {
		...
		if err != nil {
			return nil, fmt.Errorf("slides: encode request: %w", err)
		}
		payloadBytes = b
	}
	for attempt := 0; ; attempt++ {
		...
		if status < 200 || status > 299 {
			...
			apiErr := fmt.Errorf("slides API error (HTTP %d): %s%s", status, apiMessage(body), hint)
...
func (s *Service) doRequest(ctx context.Context, token, method, endpoint, path string, payload []byte) (int, []byte, error) {
	...
	if err != nil {
		return 0, nil, fmt.Errorf("slides: build request: %w", err)
	}
	...
	if err != nil {
		return 0, nil, fmt.Errorf("slides: %s %s: %w", method, path, err)
	}
	defer resp.Body.Close()
	body, err := io.ReadAll(resp.Body)
	if err != nil {
		return 0, nil, fmt.Errorf("slides: read response: %w", err)
	}
...
func (s *Service) emit(body []byte) error {
	...
	if !json.Valid(body) {
		return fmt.Errorf("slides: provider returned invalid JSON")
	}
...
func (s *Service) emitJSON(value any) error {
	...
	if err != nil {
		return fmt.Errorf("slides: encode output: %w", err)
	}
	return s.emit(body)
}
