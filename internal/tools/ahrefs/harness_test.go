...
// capturedRequest records what the fake Ahrefs server saw.
...
// run executes one ahrefs command against srv with a fixed seed token.
...
// runNoToken executes with an empty credential map to exercise the pre-parse
// missing-token path.
