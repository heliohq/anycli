...
const (
	testToken    = "tok-123"
	testClientID = "cid-abc"
)

// capturedRequest records what the fake Twitch server saw.
...
// and appends to order so self-resolution flows can be asserted.
func newMultiServer(t *testing.T, routes map[string]routeHandler, captured map[string]capturedRequest, order *[]string) *httptest.Server {
	t.Helper()
	var mu sync.Mutex
	return httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		body, _ := io.ReadAll(r.Body)
		...
		if !ok {
			...
			_, _ = w.Write([]byte(`{"error":"Not Found","status":404,"message":"no route"}`))
			return
		}
		w.Header().Set("Content-Type", "application/json")
...
func runResult(t *testing.T, srv *httptest.Server, args ...string) (execution.Result, string, string) {
	...
	env := map[string]string{EnvToken: testToken, EnvClientID: testClientID}
...
// decodeOut unmarshals stdout into a generic map.
func decodeOut(t *testing.T, out string) map[string]any {
	t.Helper()
	var m map[string]any
	if err := json.Unmarshal([]byte(out), &m); err != nil {
		t.Fatalf("stdout not a JSON object: %v (%s)", err, out)
	}
	return m
}
