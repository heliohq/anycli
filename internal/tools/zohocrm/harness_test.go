...
// newMux is a multi-route fake Zoho CRM server: it answers each request from
// routes keyed by "METHOD /path" (the path is the full request path, including
// the /crm/v8 prefix) and records every request into reqs. An unmatched route
// returns 404 with a Zoho-shaped error body.
func newMux(t *testing.T, reqs *[]capturedRequest, routes map[string]stub) *httptest.Server {
	...
	return httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		*reqs = append(*reqs, capturedRequest{
		...
		_, _ = w.Write([]byte(`{"code":"RESOURCE_NOT_FOUND","message":"not found","status":"error"}`))
	}))
}
