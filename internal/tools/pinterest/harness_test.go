...
// capturedRequest records what the fake Pinterest server saw.
...
// run executes the service against srv and returns exit code + streams.
...
func runResult(t *testing.T, srv *httptest.Server, args ...string) (execution.Result, string, string) {
	...
	result, err := svc.Execute(context.Background(), args, map[string]string{EnvAccessToken: "pina-tok-123"})
	if err != nil {
