...
// capturedRequest records what the fake Customer.io server saw.
...
func newMultiServer(t *testing.T, routes map[string]routeHandler, captured map[string]capturedRequest) *httptest.Server {
	...
	return httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if !ok {
			...
			_, _ = w.Write([]byte(`{"meta":{"error":"NOT_FOUND"}}`))
			return
		}
		w.Header().Set("Content-Type", "application/json")
...
// run executes one customer-io command against the fake server with a seeded
// API key, returning exit code and captured streams.
