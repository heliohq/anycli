...
// capturedRequest records what the fake Novu server saw.
...
// run executes one novu invocation against the fake server with a seeded
// secret, returning exit code and captured stdout/stderr.
...
func runResult(t *testing.T, srv *httptest.Server, args ...string) (execution.Result, string, string) {
	...
	env := map[string]string{EnvSecretKey: "tok-123"}
