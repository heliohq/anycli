...
// capturedRequest records what the fake Brevo server saw.
...
// routeHandler is one route's canned reply on a multi-route server.
...
func newMultiServer(t *testing.T, routes map[string]routeHandler, captured map[string]capturedRequest) *httptest.Server {
	...
	return httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if !ok {
			...
			_, _ = w.Write([]byte(`{"code":"document_not_found","message":"not found"}`))
			return
		}
		w.Header().Set("Content-Type", "application/json")
