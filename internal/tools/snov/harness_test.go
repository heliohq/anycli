...
// capturedRequest records what the fake Snov server saw for one request.
...
// fakeSnov is a path-routed fake Snov API. Each path serves a queue of
// responses (popped per hit; the last reply repeats once the queue drains) so a
// test can script an async task as "in progress" then "completed". Every
// request is recorded in order.
type fakeSnov struct {
	mu       sync.Mutex
	queues   map[string][]cannedResponse
	requests []capturedRequest
}

func newFake() *fakeSnov {
	return &fakeSnov{queues: map[string][]cannedResponse{}}
}

// on queues one or more responses for a path (served in order).
func (f *fakeSnov) on(path string, responses ...cannedResponse) *fakeSnov {
	f.queues[path] = append(f.queues[path], responses...)
	return f
}

func (f *fakeSnov) serve(t *testing.T) *httptest.Server {
	t.Helper()
	return httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case len(queue) == 0:
			...
			_, _ = w.Write([]byte(`{"message":"no canned response for ` + r.URL.Path + `"}`))
			return
		case len(queue) == 1:
...
func (f *fakeSnov) hits(path string) []capturedRequest {
	f.mu.Lock()
	defer f.mu.Unlock()
...
func (f *fakeSnov) first(t *testing.T, path string) capturedRequest {
	t.Helper()
	got := f.hits(path)
	if len(got) == 0 {
...
func (f *fakeSnov) withToken() *fakeSnov {
	return f.on(tokenPath, cannedResponse{http.StatusOK, okToken})
}

// runSnov executes a snov command against the fake with both secrets set and a
// tiny poll interval so async loops finish instantly.
func runSnov(t *testing.T, srv *httptest.Server, args ...string) (execution.Result, string, string) {
	t.Helper()
