...
// capturedRequest records what the fake Resend server saw.
...
func newMultiServer(t *testing.T, routes map[string]routeHandler, captured map[string]capturedRequest) *httptest.Server {
	...
	return httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if !ok {
			...
			_, _ = w.Write([]byte(`{"name":"not_found","message":"NOT_FOUND"}`))
			return
		}
		w.Header().Set("Content-Type", "application/json")
...
func runResult(t *testing.T, srv *httptest.Server, args ...string) (execution.Result, string, string) {
	...
	result, err := svc.Execute(context.Background(), args, map[string]string{EnvAPIKey: "re_test-key"})
	if err != nil {
...
// decodeArrayBody unmarshals a captured request body into a generic slice.
func decodeArrayBody(t *testing.T, raw []byte) []any {
	t.Helper()
	var a []any
	if err := json.Unmarshal(raw, &a); err != nil {
		t.Fatalf("request body not JSON array: %v (%s)", err, raw)
	}
	return a
}
