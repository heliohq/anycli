...
// capturedRequest records what the fake Lusha server saw.
...
// runResult executes one lusha command against the fake server with a fixed key.
...
// run is runResult keeping only the exit code + streams.
...
// decodeStdout unmarshals the tool's stdout envelope into a generic map.
func decodeStdout(t *testing.T, out string) map[string]any {
	t.Helper()
	var m map[string]any
