...
// capturedRequest records what the fake Hunter server saw.
type capturedRequest struct {
	...
	Auth   string // Authorization header (must stay empty)
	Body   []byte
}
...
func newMultiServer(t *testing.T, routes map[string]routeHandler, captured map[string]capturedRequest) *httptest.Server {
	...
	return httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if !ok {
			...
			_, _ = w.Write([]byte(`{"errors":[{"id":"not_found","details":"no route"}]}`))
			return
		}
		w.Header().Set("Content-Type", "application/json")
