...
// capturedRequest records what the fake Typefully server saw.
...
// newHeaderServer is like newServer but sets extra response headers (used for
// the 429 rate-limit reset assertion).
func newHeaderServer(t *testing.T, status int, response string, headers map[string]string, got *capturedRequest) *httptest.Server {
	...
	return httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		body, _ := io.ReadAll(r.Body)
		*got = capturedRequest{Method: r.Method, Path: r.URL.Path, Query: r.URL.RawQuery, Auth: r.Header.Get("Authorization"), Body: body}
...
func runResult(t *testing.T, srv *httptest.Server, args ...string) (execution.Result, string, string) {
	...
	result, err := svc.Execute(context.Background(), args, map[string]string{EnvAPIKey: "key-123"})
	if err != nil {
