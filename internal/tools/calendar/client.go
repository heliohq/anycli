...
// call performs one Calendar API request with Bearer auth. Non-2xx surfaces the
...
// 429/5xx status. Non-GET requests are never auto-retried: a POST/PATCH may
// have been applied even when the response is a 5xx, and re-sending would
...
func (s *Service) call(ctx context.Context, token, method, path string, query url.Values, payload any) ([]byte, error) {
	...
	if payload != nil {
		...
		if err != nil {
			return nil, fmt.Errorf("calendar: encode request: %w", err)
		}
		payloadBytes = b
	}
	for attempt := 0; ; attempt++ {
		...
		if status < 200 || status > 299 {
			...
			apiErr := fmt.Errorf("calendar API error (HTTP %d): %s%s", status, apiMessage(body), hint)
			return nil, classifyCredentialError(status, body, apiErr)
		}
		// Some responses (DELETE, or PATCH with sendUpdates) return an empty
		// 2xx body; that is a legitimate success, not a retry trigger.
...
func (s *Service) doRequest(ctx context.Context, token, method, endpoint, path string, payload []byte) (int, []byte, error) {
	...
	if err != nil {
		return 0, nil, fmt.Errorf("calendar: build request: %w", err)
	}
	...
	if err != nil {
		return 0, nil, fmt.Errorf("calendar: %s %s: %w", method, path, err)
	}
	defer resp.Body.Close()
	body, err := io.ReadAll(resp.Body)
	if err != nil {
		return 0, nil, fmt.Errorf("calendar: read response: %w", err)
	}
...
// sanitizeJSON escapes raw control characters (bytes < 0x20) that appear inside
// JSON string literals — event summaries/descriptions occasionally carry them —
...
func (s *Service) emit(body []byte) error {
	...
	if !json.Valid(body) {
		return fmt.Errorf("calendar: provider returned invalid JSON")
	}
...
func (s *Service) emitJSON(value any) error {
	...
	if err != nil {
		return fmt.Errorf("calendar: encode output: %w", err)
	}
	return s.emit(body)
}
