...
// retryBackoffs are the delays before each automatic GET retry (the People API
// intermittently returns transient 429/5xx or empty 2xx bodies under rapid
// sequential calls). Length bounds the retry count.
var retryBackoffs = []time.Duration{200 * time.Millisecond, 800 * time.Millisecond}

// call performs one People API request with Bearer auth. Non-2xx surfaces the
...
// 429/5xx status or on a 2xx response with an empty body. Non-GET requests are
// never auto-retried (v1 is read-only, so in practice every call is a GET, but
// the guard stays for parity with the shared pattern).
func (s *Service) call(ctx context.Context, token, method, path string, query url.Values, payload any) ([]byte, error) {
	...
	if payload != nil {
		...
		if err != nil {
			return nil, fmt.Errorf("contacts: encode request: %w", err)
		}
		payloadBytes = b
	}
	for attempt := 0; ; attempt++ {
		...
		if status < 200 || status > 299 {
			...
			apiErr := fmt.Errorf("contacts API error (HTTP %d): %s%s", status, apiMessage(body), hint)
			return nil, classifyCredentialError(status, body, apiErr)
		}
		if method == http.MethodGet && len(bytes.TrimSpace(body)) == 0 {
			return nil, fmt.Errorf("contacts: GET %s: empty response from API (HTTP %d) after %d attempts", path, status, attempt+1)
		}
		return body, nil
	}
}
...
func (s *Service) doRequest(ctx context.Context, token, method, endpoint, path string, payload []byte) (int, []byte, error) {
	...
	if err != nil {
		return 0, nil, fmt.Errorf("contacts: build request: %w", err)
	}
	...
	if err != nil {
		return 0, nil, fmt.Errorf("contacts: %s %s: %w", method, path, err)
	}
	defer resp.Body.Close()
	body, err := io.ReadAll(resp.Body)
	if err != nil {
		return 0, nil, fmt.Errorf("contacts: read response: %w", err)
	}
...
// pause sleeps for the retry/warmup delay; tests inject a recorder via s.sleep.
...
func (s *Service) emit(body []byte) error {
	...
	if !json.Valid(body) {
		return fmt.Errorf("contacts: provider returned invalid JSON")
	}
...
func (s *Service) emitJSON(value any) error {
	...
	if err != nil {
		return fmt.Errorf("contacts: encode output: %w", err)
	}
	return s.emit(body)
}
