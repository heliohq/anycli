...
// call performs one Tasks API request with Bearer auth. Non-2xx surfaces the
...
// 429/5xx status. Non-GET requests are never auto-retried: a POST/PATCH/DELETE
// may have been applied even when the response is a 5xx, and re-sending would
// double the side effect (Tasks delete is irreversible).
//
// A 2xx with an empty body is valid here: tasks.delete, tasklists.delete, and
// tasks.clear all return 204 No Content. The caller decides whether it needs a
// body.
func (s *Service) call(ctx context.Context, token, method, path string, query url.Values, payload any) ([]byte, error) {
	...
	if payload != nil {
		...
		if err != nil {
			return nil, fmt.Errorf("tasks: encode request: %w", err)
		}
		payloadBytes = b
	}
	for attempt := 0; ; attempt++ {
		...
		if status < 200 || status > 299 {
			...
			apiErr := fmt.Errorf("tasks API error (HTTP %d): %s%s", status, apiMessage(body), hint)
...
func (s *Service) doRequest(ctx context.Context, token, method, endpoint, path string, payload []byte) (int, []byte, error) {
	...
	if err != nil {
		return 0, nil, fmt.Errorf("tasks: build request: %w", err)
	}
	...
	if err != nil {
		return 0, nil, fmt.Errorf("tasks: %s %s: %w", method, path, err)
	}
	defer resp.Body.Close()
	body, err := io.ReadAll(resp.Body)
	if err != nil {
		return 0, nil, fmt.Errorf("tasks: read response: %w", err)
	}
...
func (s *Service) emit(body []byte) error {
	...
	if !json.Valid(body) {
		return fmt.Errorf("tasks: provider returned invalid JSON")
	}
...
func (s *Service) emitJSON(value any) error {
	...
	if err != nil {
		return fmt.Errorf("tasks: encode output: %w", err)
	}
	return s.emit(body)
}
