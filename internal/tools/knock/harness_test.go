...
// capturedRequest records what the fake Knock server saw.
...
// run executes one knock invocation against srv with a fixed test key.
...
func runResult(t *testing.T, srv *httptest.Server, args ...string) (execution.Result, string, string) {
	...
	result, err := svc.Execute(context.Background(), args, map[string]string{EnvAPIKey: "sk_test_123"})
	if err != nil {
