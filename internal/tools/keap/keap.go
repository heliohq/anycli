// Package keap is the built-in Keap (formerly Infusionsoft) CRM service: a
// non-interactive cobra tree over the Keap REST v2 surface
// (https://api.infusionsoft.com/crm/rest, /v2/... paths). Auth is
// "Authorization: Bearer <access_token>" from an OAuth 2.0 access token. Keap
// fails with a non-2xx status and a JSON body carrying a message field; every
// command emits the provider JSON on stdout (passthrough + newline) and a
// one-line error on stderr, with a structured envelope under --json. A 401
// rejects the credential so the engine can stale-mark it.
package keap
...
// DefaultBaseURL is the production Keap REST base. The /v2 segment is part of
// each request path, not the base, matching the official OpenAPI servers entry.
const DefaultBaseURL = "https://api.infusionsoft.com/crm/rest"
...
type Service struct {
	// BaseURL overrides the Keap API base; empty = DefaultBaseURL. Tests point
...
// Execute runs one keap subcommand with the resolved credentials in env.
// Success is exit 0; usage/param errors (missing required flags, bad enums,
// invalid JSON, unknown subcommands) are exit 2; runtime/API errors (Keap
...
func (s *Service) newRoot(token string) *cobra.Command {
	...
	root.PersistentFlags().Bool("json", false, "force structured JSON error output (success is always JSON passthrough)")
