...
// capturedRequest records what the fake Gorgias server saw.
...
// run executes one gorgias invocation against the fake server with a seeded
// access token, returning the exit code and captured streams.
...
func runResult(t *testing.T, srv *httptest.Server, args ...string) (execution.Result, string, string) {
	...
	env := map[string]string{EnvAccessToken: "tok-123", EnvSubdomain: "acme"}
