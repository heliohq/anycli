...
// classifyCredentialError marks a response as an explicit credential rejection
// only for genuine auth failures (HTTP 401 or an UNAUTHENTICATED / authError
// body). A 403 PERMISSION_DENIED (missing scope) leaves the credential valid —
// the fix is reconnect for scope, not credential invalidation.
