...
// classifyCredentialError marks the error as a credential rejection only when
// the provider explicitly rejects the token (401 or an UNAUTHENTICATED /
// authError body). A 403 PERMISSION_DENIED (missing scope) leaves the
// credential valid — the fix is a reconnect, not a re-mint.
