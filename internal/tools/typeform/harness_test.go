...
// capturedRequest records one request the fake Typeform server received.
...
// newMux is a multi-route fake Typeform server: it answers each request from
// routes keyed by "METHOD /path" and records every request into reqs. An
// unmatched route returns 404 with a Typeform-shaped error body.
func newMux(t *testing.T, reqs *[]capturedRequest, routes map[string]stub) *httptest.Server {
	...
	return httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		*reqs = append(*reqs, capturedRequest{
		...
		_, _ = w.Write([]byte(`{"code":"NOT_FOUND","description":"not found"}`))
	}))
}

// run executes one typeform invocation against srv, capturing stdout/stderr and
// the exit code. token defaults to a fixed test token when non-empty.
func run(t *testing.T, srv *httptest.Server, token string, args ...string) (stdout, stderr string, exit int) {
	t.Helper()
	...
	return out.String(), errOut.String(), res.ExitCode
}
