...
// capturedRequest records one request the fake Omnisend server received.
...
// newMux is a multi-route fake Omnisend dated-line server: it answers each
// request from routes keyed by "METHOD /path" and records every request into
// reqs. An unmatched route returns 404 so a wrong path surfaces as a test
...
func newMux(t *testing.T, reqs *[]capturedRequest, routes map[string]stub) *httptest.Server {
	...
	return httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		*reqs = append(*reqs, capturedRequest{
			...
			Version:     r.Header.Get("Omnisend-Version"),
