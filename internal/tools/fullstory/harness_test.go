...
// capturedRequest records what the fake FullStory server saw.
...
// run executes one fullstory subcommand against the fake server with a fixed
// key and returns exit code + captured stdout/stderr.
...
func runResult(t *testing.T, srv *httptest.Server, args ...string) (execution.Result, string, string) {
	...
	result, err := svc.Execute(context.Background(), args, map[string]string{EnvAPIKey: "na1.tok-123"})
	if err != nil {
