...
// TestE2ERealGhLazyInstall exercises the REAL gh lazy-install path end to end:
// the bundled github definition's official release URL, the real sha256 pin,
// extraction into the versions/ layout, and execution of the installed
// standalone binary. Guarded by ANYCLI_E2E_DOWNLOAD=1 because it downloads
// ~40MB from github.com — CI unit runs stay offline.
func TestE2ERealGhLazyInstall(t *testing.T) {
	if os.Getenv("ANYCLI_E2E_DOWNLOAD") != "1" {
		t.Skip("set ANYCLI_E2E_DOWNLOAD=1 to run the real-download e2e")
	}
	...
	wantPrefix := filepath.Join(root, "versions", "github", def.Source.Version)
	if !strings.HasPrefix(installed, wantPrefix) {
		t.Fatalf("installed path %q not under pin layout %q", installed, wantPrefix)
	}
	if !strings.Contains(notice.String(), "installing") {
	...
	// The installed standalone binary must actually run and report the pin.
	out, err := exec.CommandContext(ctx, installed, "--version").CombinedOutput()
	if err != nil {
		t.Fatalf("exec installed gh --version: %v (out=%s)", err, out)
	}
	if !strings.Contains(string(out), def.Source.Version) {
		t.Fatalf("--version output %q does not contain pinned %q", out, def.Source.Version)
	}
